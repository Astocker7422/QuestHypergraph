...
public class PathGenerator<T, A>
{
	...
	public PathGenerator(Hypergraph<T, A> g)
    {
        this.graph = g;
    ...
    public void GeneratePathBackwardToLeaves(PathHashMap memoizedPaths,
                                             HyperedgeMultiMap<T, A> edgeDatabase,
											 int goal)
    {
    ...
    //
    private ArrayList<Path<T, A>> generateAllMaximalPathsFrom(PathHashMap memoizedPaths,
                                                        HyperedgeMultiMap<T, A> edgeDatabase, int node)
    {
        ...
        ArrayList<PebblerHyperedge<A>> edges = null;
        try {
                                                   ...
                                                   Path<T, A> basepath)
    {
        ...
        ArrayList<Path<T, A>>[] singletonMapToNewPaths = new ArrayList[givens.size()];
        ArrayList<Path<T, A>>[] singletonMapToOriginalPaths = new ArrayList[givens.size()];
        ...
        for (int g = 0; g < givens.size(); g++)
        {
            ...
            if (singletonMapToOriginalPaths[g] == null) singletonMapToOriginalPaths[g] = new ArrayList<Path<T, A>>();

            // Using the original paths, append them to the base path
            singletonMapToNewPaths[g] = new ArrayList<Path<T, A>>();

            // Append all of these given paths to the base path
            for (Path<T, A> path : singletonMapToOriginalPaths[g])
            {
                Path<T, A> basePathCopy = new Path<T, A>(basepath);
                try {
		...
		//
		ArrayList<Path<T, A>> maximalPaths = new ArrayList<Path<T, A>>(singletonMapToNewPaths[populatedIndices.get(0)]);
        populatedIndices.remove(0);  // Prime the pump with the first element.
        for (int index : populatedIndices)
        {
            ArrayList<Path<T, A>> tmpMaximalPaths = new ArrayList<Path<T, A>>();
            for (Path<T, A> singleton : singletonMapToOriginalPaths[index])
            {
                for (Path<T, A> path : maximalPaths)
                {
                    Path<T, A> pathCopy = new Path<T, A>(path);
